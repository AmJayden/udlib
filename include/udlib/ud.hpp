#pragma once
// clang format here may reorder the includes
// and cause Windows libraries to break due to precedence
// clang-format off
#include <Windows.h>
#include <SoftPub.h>
#include <algorithm>
...
#define ud_encode( str )                                                       \
    std::string_view( ud::rot::decode( ud::rot::rot_t< str >{ } ) )

#define ud_xorstr_c( str )                                                     \
    ud::xorstr::decrypt(                                                       \
        ud::xorstr::xorstr_t< str, __COUNTER__ + 1 ^ 0x90 >{ } )               \
        .data
#define ud_xorstr( str )                                                       \
    std::string_view                                                           \
    {                                                                          \
        ud::xorstr::decrypt(                                                   \
            ud::xorstr::xorstr_t< str, __COUNTER__ + 1 ^ 0x90 >{ } )           \
...
#define ud_import( mod, func )                                                 \
    reinterpret_cast< decltype( &func ) >(                                     \
        ud::lazy_import::find_module_export< TEXT( mod ), #func >( ) )
#define ud_first_import( func )                                                \
    reinterpret_cast< decltype( &func ) >(                                     \
        ud::lazy_import::find_first_export< #func >( ) )

// preprocessed settings due to MSVC (not clang or gcc) throwing errors even in
// `if constexpr` bodies
...
namespace ud
{
    namespace details
    {
        ...
        template < std::size_t sz > struct comp_string_t
        {
            std::size_t size = sz;
            char data[ sz ]{ };
        ...
        template < std::size_t sz > struct wcomp_string_t
        {
            std::size_t size = sz;
            wchar_t data[ sz ]{ };
        ...
        inline consteval std::uint64_t get_seed( )
        {
            ...
            constexpr auto time_int =
                []( const char *const str, const std::size_t len )
            {
                auto res = 0ull;
...
#undef min
    ...
    namespace rot
    {
        template < details::comp_string_t str > struct rot_t
        {
        ...
        UD_FORCEINLINE details::comp_string_t< str.size >
        decode( rot_t< str > encoded )
        {
    ...
    namespace fnv
    {
        inline constexpr std::uint32_t fnv_1a( const char *const str,
        ...
        inline constexpr std::uint32_t fnv_1a( const wchar_t *const str,
                                               const std::size_t size )
        {
            constexpr auto prime = 16777619u;
    ...
    namespace xorstr
    {
        ...
        template < details::comp_string_t str, std::uint32_t key_multiplier >
        UD_FORCEINLINE details::comp_string_t< str.size >
        decrypt( xorstr_t< str, key_multiplier > enc )
        {
            details::comp_string_t< str.size > result{ };
    ...
    {
        UD_FORCEINLINE std::uintptr_t
        get_module_handle( const std::uint64_t hash )
        {
#if defined( _M_X64 )
            const auto peb =
                reinterpret_cast< const PEB * >( __readgsqword( 0x60 ) );
#else
            const auto peb =
                reinterpret_cast< const PEB * >( __readfsdword( 0x30 ) );
#endif

            const auto modules = reinterpret_cast< const LIST_ENTRY * >(
                peb->Ldr->InMemoryOrderModuleList.Flink );

            for ( auto i = modules->Flink; i != modules; i = i->Flink )
            {
                const auto entry =
                    reinterpret_cast< const details::LDR_DATA_TABLE_ENTRY * >(
                        i );

                const auto name = entry->base_name.Buffer;
                const auto len = entry->base_name.Length;

                if ( fnv::fnv_1a( static_cast< const wchar_t * >( name ),
                                  len ) == hash )
        ...
        UD_FORCEINLINE void *
        find_primitive_export( const std::uint64_t dll_hash,
                               const std::uint64_t function_hash )
        {
            ...
            const auto exports =
                reinterpret_cast< const IMAGE_EXPORT_DIRECTORY * >(
                    module + nt->OptionalHeader
                                 .DataDirectory[ IMAGE_DIRECTORY_ENTRY_EXPORT ]
                                 .VirtualAddress );

            const auto names = reinterpret_cast< const std::uint32_t * >(
                module + exports->AddressOfNames );
            const auto ordinals = reinterpret_cast< const std::uint16_t * >(
                module + exports->AddressOfNameOrdinals );
            ...
            {
                const auto name =
                    reinterpret_cast< const char * >( module + names[ i ] );
                std::size_t len = 0;
                ...
                if ( fnv::fnv_1a( name, len ) == function_hash )
                    return reinterpret_cast< void * >(
                        module + functions[ ordinals[ i ] ] );
            }

            return nullptr;
        }

        template < details::wcomp_string_t dll_name,
                   details::comp_string_t function_name >
        UD_FORCEINLINE void *find_module_export( )
        {
            return find_primitive_export( fnv::fnv_1a< dll_name >( ),
                                          fnv::fnv_1a< function_name >( ) );
        }
        ...
        UD_FORCEINLINE void *find_first_export( )
        {
            ...
            for ( auto i = modules->Flink; i != modules; i = i->Flink )
            {
                ...
                if ( const auto exp = find_primitive_export(
                         fnv::fnv_1a( name, len ), function_hash ) )
                    return exp;
            }
    ...
    std::optional< ty > find_pattern_primitive( const std::uintptr_t start,
            ...
            else
            {
                ...
                const auto get_byte = []( const std::string &x ) -> std::uint8_t
                {
                    return static_cast< std::uint8_t >(
                        std::stoul( x, nullptr, 16 ) );
                };

                bytes.emplace_back(
                    false, get_byte( std::string( it - 1, ( ++it ) + 1 ) ) );
            }
        }

        for ( auto i = reinterpret_cast< const std::uint8_t * >( start );
              i < reinterpret_cast< const std::uint8_t * >( end ); )
        {
            auto found = true;
    ...
    struct segment_t
    {
        ...
        segment_t( const void *const module,
                   const std::string_view segment_name )
        {
            init( module, segment_name );
        }

        segment_t( const void *const handle,
    ...
    private:
        void init( const void *const handle,
                   const IMAGE_SECTION_HEADER *section )
        {
        ...
        {
            const auto dos =
                reinterpret_cast< const IMAGE_DOS_HEADER * >( handle );
            const auto nt = reinterpret_cast< const IMAGE_NT_HEADERS * >(
            ...
            for ( auto i = 0u; i < nt->FileHeader.NumberOfSections; ++i )
            {
                if ( std::string_view(
                         ...
                         8 )
                         .find( segment_name ) != std::string_view::npos )
                {
                    start = reinterpret_cast< std::uintptr_t >( handle ) +
...
#pragma code_seg( push, ".text" )
    template < auto... bytes > struct shellcode_t
    {
        static constexpr std::size_t size = sizeof...( bytes );
        __declspec( allocate( ".text" ) ) static constexpr std::uint8_t data[]{
            bytes... };
    };
#pragma code_seg( pop )

    template < typename ty, auto... bytes > constexpr ty make_shellcode( )
    {
        return reinterpret_cast< const ty >( &shellcode_t< bytes... >::data );
    }

    template < std::uint8_t... bytes > UD_FORCEINLINE constexpr void emit( )
    {
    ...
    template < std::size_t size, std::uint32_t seed = __COUNTER__ + 0x69,
               std::size_t count = 0 >
    UD_FORCEINLINE constexpr void emit_random( )
    {
        if constexpr ( count < size )
        {
            ...
            emit_random< size, static_cast< std::uint32_t >( random ) * seed,
                         count + 1 >( );
        }
    }

    inline bool is_valid_page( const void *const data,
                               const std::uint32_t flags = PAGE_READWRITE )
    {
    ...
    struct module_t
    {
        ...
        {
            const auto dos =
                reinterpret_cast< const IMAGE_DOS_HEADER * >( start );
            const auto nt = reinterpret_cast< const IMAGE_NT_HEADERS * >(
                start + dos->e_lfanew );
            ...
            const auto export_dir =
                reinterpret_cast< const IMAGE_EXPORT_DIRECTORY * >(
                    start + directory_header.VirtualAddress );
            const auto name_table = reinterpret_cast< const std::uint32_t * >(
                start + export_dir->AddressOfNames );
            const auto ord_table = reinterpret_cast< const std::uint16_t * >(
                start + export_dir->AddressOfNameOrdinals );
            const auto addr_table = reinterpret_cast< const std::uint32_t * >(
                start + export_dir->AddressOfFunctions );
            ...
            {
                const auto name_str =
                    reinterpret_cast< const char * >( start + name_table[ i ] );
            ...
            for ( auto i = 0u;; ++i )
            {
                ...
                const auto name_table =
                    reinterpret_cast< const std::uint32_t * >(
                        start + directory->OriginalFirstThunk );
                const auto addr_table =
                    reinterpret_cast< const std::uint32_t * >(
                        start + directory->FirstThunk );
                ...
                for ( auto j = 0u;; ++j )
                {
                    ...
                    // however, it's faster than querying that page protection
                    // to see if it's readable
                    __try
                    {
                        name = name_ptr;
                    }
                    ...
                    name_str = is_valid_page( name_ptr, PAGE_READONLY )
                                   ? name_ptr
                                   : "";
#endif

                    // emplace_back doesn't allow for implicit conversion, so we
                    // have to do it manually
                    imports.push_back(
                        { name_str, static_cast< std::uint16_t >( j ),
        ...
        get_strings( const std::size_t minimum_size = 0 ) const
        {
            ...
            const auto start =
                reinterpret_cast< const std::uint8_t * >( rdata.start );
            const auto end =
                reinterpret_cast< const std::uint8_t * >( rdata.end );
        ...
        bool is_signed( )
        {
            ...
            if ( !GetModuleFileNameW( reinterpret_cast< HMODULE >( module ),
                                      buffer, MAX_PATH ) )
                return false;

            WINTRUST_FILE_INFO f_data{ .cbStruct = sizeof( WINTRUST_FILE_INFO ),
                                       .pcwszFilePath = buffer };

            auto policy_guid = GUID( WINTRUST_ACTION_GENERIC_VERIFY_V2 );

            WINTRUST_DATA wt_data{ .cbStruct = sizeof( WINTRUST_DATA ),
            ...
            const auto valid = WinVerifyTrust( nullptr, &policy_guid,
                                               &wt_data ) == ERROR_SUCCESS;

            wt_data.dwStateAction = WTD_STATEACTION_CLOSE;
            WinVerifyTrust( nullptr, &policy_guid, &wt_data );

            return valid;
        }

        module_t( ) { init( GetModuleHandle( nullptr ) ); }

        explicit module_t( void *const handle ) { init( handle ); }
    ...
    private:
        ...
        void init( void *const handle )
        {
            ...
            const auto sz = GetModuleFileNameA(
                static_cast< HMODULE >( handle ), buffer, MAX_PATH );
    ...
    inline std::vector< module_t > get_modules( )
    {
        ...
        {
            const auto entry =
                reinterpret_cast< const LDR_DATA_TABLE_ENTRY * >( i );

            if ( entry->Reserved2[ 0 ] || entry->DllBase )
                result.emplace_back( entry->Reserved2[ 0 ]
                                         ? entry->Reserved2[ 0 ]
                                         : entry->DllBase );
        }
    ...
    inline std::optional< module_t >
    get_module_at_address( const std::uintptr_t address )
    {
    ...
    ty calculate_relative( const std::uintptr_t address,
                           const std::uint8_t size, const std::uint8_t offset )
    {
        return ty( address + *reinterpret_cast< rel_t * >( address + offset ) +
                   size );
    }
} // namespace ud

template < std::size_t size >
UD_FORCEINLINE std::ostream &
operator<<( std::ostream &os, const ud::details::comp_string_t< size > &str )
{
